...
export default function PlayerRow({ player, onBuyIn, onRemove }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
      ...
      {confirmingRemove && (
        <div className="border-t border-gray-100 bg-red-50 p-3 space-y-2">
          <p className="text-xs text-red-600 font-medium">Enter passphrase to remove {player.name}:</p>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-red-400"
              autoFocus
            />
            <button
              type="button"
            ...
            <button
              type="button"
              onClick={() => { setConfirmingRemove(false); setPassphrase(''); setError(''); }}
